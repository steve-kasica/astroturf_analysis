---
title: "Finding Suspicious Clusters with GLoVe and HDBSCAN"
output: html_notebook
---

This notebook is where I perform essentially the same exploratory data analysis and manual tagging that Jeff Kao did on the FCC 17-108 dataset for the sake of replicability, except all the calculations in this notebook are done with R.
...
```{r}
comments <- read_csv('./data/sample_10000_42.csv')
comments['X1'] <- NULL  # first column is an error

# Take a sample of the 100,000 comments vectorized with GLoVe
txt <- comments[c('docid', 'text_data')]
head(comments)
```

```{r}
comments <- add_column(comments, cluster=cl$cluster, .after='docid')
```

```{r}
cl_tbl <- as.data.frame(table(cl$cluster))
colnames(cl_tbl) <- c('Cluster', 'Freq')
cl_tbl <- cl_tbl[order(-cl_tbl$Freq),]
kable(cl_tbl, caption = 'Cluster Frequency')
```

```{r}
ggplot(cl_tbl, aes(x=Cluster, y=Freq)) + geom_bar(stat='identity')
```

Cluster 0 will be discarded since it represents unclustered comments. The top clusters
```{r}
cluster_ids = c(12, 6, 32)
comments %>% 
  filter(cluster %in% cluster_ids) %>% 
  select(docid, cluster, text_data) %>%
  arrange(cluster)
```
...
Cluster 3
<blockquote><mark>I am in favor of strong net neutrality under Title II of the Telecommunications Act.</mark>

Sincerely,

Ryan Isaac</blockquote> 

Cluster 17, docid 1185246
<blockquote><mark>As an Internet user, I'm asking the FCC to protect the net neutrality protections currently in place.

The FCC should throw out Chairman Ajit Pai's proposal to give the ISP monopolies like Comcast, AT&T, and Verizon the authority to create Internet fast lanes, stripping Internet users of the meaningful access and privacy protections we fought for and just recently won.</mark>
...
Cluster 20, docid 10185258
<blockquote>

This is a lengthy automatic letter. But guys. C'mon. Obviously. Just c'mon. Don't be silly.

<mark>The FCC's Open Internet Rules (net neutrality rules) are extremely important to me</mark>. I urge you to protect them.
...
<mark>As an Etsy seller, net neutrality is essential to the success of my business and my ability to care for myself and my family.</mark> The FCC needs to ensure equal opportunities for microbusinesses to compete with larger and more established brands by upholding net neutrality protections.
